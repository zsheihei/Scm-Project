...
namespace app\scm\controller\setting;
...
use think\Request;
use think\Url;
use app\scm\model\system\SystemMenus as MenusModel;
...
//            ['access',''],
    ...
    public function create($cid = 0)
    {
        $form = Form::create(Url::build('save'),[
            Form::input('menu_name','菜单名称')->required('菜单名称必填'),
            ...
            Form::select('module','模块名')->options([['label'=>'系统模块','value'=>'scm']]),
            Form::input('controller','控制器名'),
            Form::input('action','方法名'),
            Form::input('params','参数')->placeholder('举例:a/123/b/234'),
            Form::frameInputOne('icon','图标',Url::build('scm/widget.widgets/icon',array('fodder'=>'icon')))->icon('ionic'),
            Form::number('sort','排序',0),
            Form::radio('is_show','是否显示',1)->options([['value'=>0,'label'=>'隐藏'],['value'=>1,'label'=>'显示(菜单只显示三级)']]),
        ]);
    ...
    public function save(Request $request)
    {
        $data = Util::postMore([
            ...
            ['is_show',0],
            ['access',1]],$request);
        if(!$data['menu_name']) return Json::fail('请输入菜单名称');
        MenusModel::set($data);
    ...
    public function edit($id)
    {
        ...
        $form = Form::create(Url::build('update',array('id'=>$id)),[
            Form::input('menu_name','菜单名称',$menu['menu_name']),
            ...
            Form::select('module','模块名',$menu['module'])->options([['label'=>'系统模块','value'=>'scm']]),
            ...
            Form::frameInputOne('icon','图标',Url::build('scm/widget.widgets/icon',array('fodder'=>'icon')),$menu['icon'])->icon('ionic'),
            Form::number('sort','排序',$menu['sort']),
            Form::radio('is_show','是否显示',$menu['is_show'])->options([['value'=>0,'label'=>'隐藏'],['value'=>1,'label'=>'显示(菜单只显示三级)']])
        ]);
